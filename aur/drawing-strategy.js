...
class AuroraDrawing {
    ...
    draw(ctx, aurora) {
        ...
        grad.addColorStop(0.6, "rgba(" + aurora.color.join(",") + ",0.1)");
        grad.addColorStop(1, "rgba(" + aurora.color.join(",") + ",0.25)");
        ctx.fillStyle = grad;

        // parameters
        const pointsCount =1;// mt_rand(10,80); // number of points along the bottom
